...
# ── Query intent / resume-retrieval tests ────────────────────────────


class MockResumeStore:
    """Mock store with resume chunks — header chunk NOT naturally retrieved."""

    HEADER_CHUNK = Chunk(
        text="John Doe\njohn.doe@gmail.com\n+1 555 123 4567\nSoftware Engineer with 5 years experience",
        page_num=1,
        source_file="resume_john.pdf",
        chunk_id="res_p1_header",
        document_id="doc_resume",
    )
    SKILLS_CHUNK = Chunk(
        text="Skills: Python, Java, TypeScript, React, Node.js, Docker, Kubernetes, AWS, PostgreSQL, Redis",
        page_num=1,
        source_file="resume_john.pdf",
        chunk_id="res_p1_skills",
        document_id="doc_resume",
    )
    PROJ1_CHUNK = Chunk(
        text="Project: Cloud Migration Platform. Migrated 50+ microservices to AWS EKS. Reduced deployment time by 80%. Tech: AWS, Docker, Terraform.",
        page_num=1,
        source_file="resume_john.pdf",
        chunk_id="res_p1_proj1",
        document_id="doc_resume",
    )
    PROJ2_CHUNK = Chunk(
        text="Project: Real-time Chat Application. Built WebSocket-based chat with Redis pub/sub. 100K concurrent users. Tech: Node.js, Redis, Socket.IO.",
        page_num=1,
        source_file="resume_john.pdf",
        chunk_id="res_p1_proj2",
        document_id="doc_resume",
    )
    PROJ3_CHUNK = Chunk(
        text="Project: ML Pipeline Orchestrator. Designed end-to-end ML training pipeline with Airflow. Reduced training time by 40%. Tech: Python, Airflow, MLflow.",
        page_num=1,
        source_file="resume_john.pdf",
        chunk_id="res_p1_proj3",
        document_id="doc_resume",
    )
    EXP_CHUNK = Chunk(
        text="Experience: Senior Software Engineer at Acme Corp (2020-2025). Led team of 8 engineers. Architected distributed system handling 1M+ requests/day.",
        page_num=1,
        source_file="resume_john.pdf",
        chunk_id="res_p1_exp",
        document_id="doc_resume",
    ...
    ALL_CHUNKS = [HEADER_CHUNK, SKILLS_CHUNK, PROJ1_CHUNK, PROJ2_CHUNK, PROJ3_CHUNK, EXP_CHUNK]

    def similarity_search(self, query, k=4, score_threshold=0.3, document_id=None):
        """Return chunks without the header — simulates embedding mismatch for name queries."""
        non_header = [c for c in self.ALL_CHUNKS if c.chunk_id != "res_p1_header"]
        results = []
        for c in non_header[:k]:
            results.append(SearchResult(chunk=c, score=0.85))
        return results
...
def test_detect_query_intent_identity():
    """Verify identity queries are detected."""
    chain = RAGChain(store=MockStore(), llm=MockLLM())
    assert chain._detect_query_intent("what is the name?") == "identity"
    assert chain._detect_query_intent("who is the candidate") == "identity"
    assert chain._detect_query_intent("email address") == "identity"
    assert chain._detect_query_intent("phone number") == "identity"
    assert chain._detect_query_intent("contact info") == "identity"
    assert chain._detect_query_intent("what is your name") == "identity"


def test_detect_query_intent_list():
    """Verify list/enumeration queries are detected."""
    chain = RAGChain(store=MockStore(), llm=MockLLM())
    assert chain._detect_query_intent("list the projects") == "list"
    assert chain._detect_query_intent("what are the skills") == "list"  # matches on "skills"
    assert chain._detect_query_intent("tell me about experience") == "list"  # matches on "experience"
    assert chain._detect_query_intent("what certifications do you have") == "list"  # matches on "certifications"
    assert chain._detect_query_intent("list projects") == "list"


def test_detect_query_intent_default():
    """Verify non-matching queries return default."""
    chain = RAGChain(store=MockStore(), llm=MockLLM())
    assert chain._detect_query_intent("what is a black hole") == "default"
    assert chain._detect_query_intent("explain quantum computing") == "default"


def test_identity_query_injects_header_chunk():
    """Verify identity queries inject the first-page chunk into results."""
    chain = RAGChain(store=MockResumeStore(), llm=MockLLM())
    chunks = chain._retrieve("what is the name in this resume?")
    texts = [c.chunk.text for c in chunks]
    header_text = MockResumeStore.HEADER_CHUNK.text
    assert header_text in texts, (
        f"Header chunk should be injected for identity query.\n"
        f"Got chunks:\n" + "\n".join(texts)
    )


def test_non_identity_queries_dont_inject_header():
    """Verify non-identity queries do NOT inject the header chunk."""
    chain = RAGChain(store=MockResumeStore(), llm=MockLLM())
    chunks = chain._retrieve("what is a black hole?")
    texts = [c.chunk.text for c in chunks]
    header_text = MockResumeStore.HEADER_CHUNK.text
    # top_k=4, store has 5 non-header chunks; all 4 should be from non-header
    assert header_text not in texts, "Header should NOT be injected for non-identity query"


def test_ensure_identity_in_context_already_present():
    """Verify no injection when header is already in results."""
    chain = RAGChain(store=MockStore(), llm=MockLLM())
    # Create results that already contain an email (identity info)
    results = [
        SearchResult(
            chunk=Chunk(
                text="Contact: john@company.com, Phone: 555-0000",
                page_num=1,
        ...
        SearchResult(
            chunk=Chunk(text="Other content", page_num=1, source_file="resume.pdf", chunk_id="o1", document_id="doc"),
            score=0.5,
        ),
    ]
    out = chain._ensure_identity_in_context(results, document_id=None)
    assert len(out) == 2
    # Should still be the same results (no injection needed)
    assert out[0].chunk.text == results[0].chunk.text


def test_ensure_identity_in_context_empty():
    """Verify no crash with empty results."""
    chain = RAGChain(store=MockStore(), llm=MockLLM())
    out = chain._ensure_identity_in_context([], document_id=None)
    assert out == []


def test_expand_query_identity():
    """Verify identity query expansion appends relevant keywords."""
    chain = RAGChain(store=MockStore(), llm=MockLLM())
    expanded = chain._expand_query("what is the name?", "identity")
    assert "name" in expanded
    assert "email" in expanded
    assert "header" in expanded


def test_expand_query_list():
    """Verify list query expansion appends relevant keywords."""
    chain = RAGChain(store=MockStore(), llm=MockLLM())
    expanded = chain._expand_query("list projects", "list")
    assert "list" in expanded
    assert "details" in expanded


def test_expand_query_default():
    """Verify default queries are not expanded."""
    chain = RAGChain(store=MockStore(), llm=MockLLM())
    expanded = chain._expand_query("what is a black hole?", "default")
    assert expanded == "what is a black hole?"


def test_list_query_expands_candidate_k(monkeypatch):
    """Verify list queries use expanded query for broader retrieval."""
    monkeypatch.setattr("src.rag.chain.settings.retrieval.enable_hybrid", False)
    monkeypatch.setattr("src.rag.chain.settings.retrieval.enable_reranker", False)
    monkeypatch.setattr("src.rag.chain.settings.retrieval.top_k", 4)

    chain = RAGChain(store=MockResumeStore(), llm=MockLLM())
    chunks = chain._retrieve("list all projects")
    # Should retrieve k=4 chunks (unchanged top_k)
    assert len(chunks) == 4
